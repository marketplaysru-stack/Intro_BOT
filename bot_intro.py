...
# -*- coding: utf-8 -*-
"""
Интро-бот – публикует на личную стену ВК анонсы только наших навигаторов.
Автопостинг каждые 6 часов, авто-комментарий со ссылками на все группы.
Картинка для анонса музыкального навигатора берётся из последнего поста его группы.
Для остальных – из Pexels или баннер.
"""
...
GROUPS = [
    ...
    },
    {
        ...
        "description": "помощь музыкантам, авторам и исполнителям в развитии",
        "theme": "музыка и творчество",
        "hashtags": "#музыка #творчество #музыканты #песни #развитие",
        "group_id": 240656847  # реальный ID группы (публичная страница)
    }
]
...
def generate_intro(group):
    ...
    if AGNES_API_KEY:
        try:
            ...
            prompt = f"""
Напиши короткий интригующий пост-анонс для группы {name} ({link}), где мы публикуем контент о {theme}.
...
def get_last_post_image(group_id):
    """
    Получает URL первой фотографии из последнего поста группы/публичной страницы.
    Если поста нет или фото нет – возвращает None.
    """
    try:
        params = {
        ...
        if "error" in resp:
            logger.warning(f"Ошибка wall.get для группы {group_id}: {resp['error']['error_msg']}")
            return None
        items = resp.get("response", {}).get("items", [])
        if not items:
            logger.info(f"В группе {group_id} нет постов")
            return None
        post = items[0]
        ...
        logger.info(f"В последнем посте группы {group_id} нет фото")
        return None
    except Exception as e:
        logger.error(f"Ошибка получения фото из поста группы {group_id}: {e}")
        return None
...
def generate_image(group):
    """
    Генерирует картинку для анонса.
    Если у группы указан group_id – пытается взять фото из последнего поста.
    Если не получилось – ищет по теме в Pexels.
    """
    # 1. Пробуем взять из поста группы
    if group.get("group_id"):
        img_url = get_last_post_image(group["group_id"])
        if img_url:
            img = download_photo(img_url)
            if img:
                logger.info(f"✅ Картинка взята из последнего поста группы {group['name']}")
                return img, "из поста группы"

    # 2. Если не получилось – используем Pexels по теме
    theme = group["theme"]
    if PEXELS_API_KEY:
        ...
        if photo_url:
            ...
            if img:
                logger.info(f"✅ Картинка от Pexels для {group['name']}")
                return img, "Pexels"
    ...
    logger.info(f"✅ Использован баннер для {group['name']}")
    return buf.getvalue(), "баннер"

# ---------- ЗАГРУЗКА ФОТО (через VkUpload с временным файлом) ----------
...
def publish_intro(group):
    try:
        ...
        image_bytes, source = generate_image(group)
        ...
        # ---- Авто-комментарий со ссылками на все навигаторы ----
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Интро-бот запущен (с картинками из постов групп)")
