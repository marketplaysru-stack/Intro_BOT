...
STANDARD_GROUPS = [
    {
        "name": "AI Навигатор",
        "link": "https://vk.ru/club240273450",
    ...
    },
    {
        ...
        "link": "https://vk.ru/club239598146",
        "theme": "строительство",
    ...
    },
    {
        ...
        "link": "https://vk.ru/club240643919",
...
MUSIC_GROUP = {
    ...
    "link": "https://vk.ru/club240656847",
...
def publish_intro(group, post_id=None, post_text=None):
    try:
        logger.info(f"📝 Начинаем публикацию анонса для {group['name']}")
        ...
        if "error" in resp:
            logger.error(f"Ошибка VK при публикации: {resp['error']['error_msg']}")
...
def scheduled_standard_post():
    ...
    try:
        ...
        logger.info(f"📌 Публикуем анонс для группы: {group['name']}")
        result = publish_intro(group)
        logger.info(f"Результат для {group['name']}: {result}")
    except Exception as e:
        logger.error(f"Ошибка в scheduled_standard_post: {e}", exc_info=True)

def standard_scheduler_worker():
    """Запускает планировщик для стандартных групп"""
    logger.info("📡 Планировщик для стандартных групп запущен (4 поста в сутки)")
    try:
        # Очищаем расписание, чтобы избежать дублирования
        schedule.clear()
        # Запускаем первый пост сразу при старте
        scheduled_standard_post()
        # Затем планируем каждые 6 часов
        schedule.every(6).hours.do(scheduled_standard_post)
        logger.info("✅ Расписание установлено: каждые 6 часов")
        while True:
            schedule.run_pending()
            time.sleep(60)
    except Exception as e:
        logger.error(f"Критическая ошибка в стандартном планировщике: {e}", exc_info=True)
        # Если упало, пытаемся перезапустить через 10 минут
        time.sleep(600)
        # Рекурсивный вызов для восстановления
        standard_scheduler_worker()
...
def monitor_music_group():
    ...
    while True:
        try:
            ...
            logger.error(f"Ошибка в мониторинге музыкальной группы: {e}", exc_info=True)
            time.sleep(600)
...
def main():
    ...
    # Запускаем планировщик для стандартных групп
    try:
        threading.Thread(target=standard_scheduler_worker, daemon=True).start()
    except Exception as e:
        logger.error(f"Ошибка запуска стандартного планировщика: {e}", exc_info=True)

    # Запускаем мониторинг музыкальной группы
    try:
        threading.Thread(target=monitor_music_group, daemon=True).start()
    except Exception as e:
        logger.error(f"Ошибка запуска мониторинга музыкальной группы: {e}", exc_info=True)
