...
# ---------- ГРУППЫ ----------
# Группы для обычных анонсов (по 1 в день)
STANDARD_GROUPS = [
    {
        "name": "AI Навигатор",
...
# Музыкальный навигатор (отслеживаем каждый пост)
MUSIC_GROUP = {
...
def generate_intro(group, post_text=None):
    ...
    # Если есть текст поста – используем его для более точного анонса
    if post_text:
        # Берём первые 100 символов как затравку
...
def get_post_image_by_id(group_id, post_id):
    """Получает URL первой фотографии из конкретного поста по его ID"""
    try:
        params = {
...
def get_last_post(group_id):
    """Возвращает словарь с id и текстом последнего поста в группе"""
    try:
        params = {
            "access_token": VK_TOKEN_USER,
            "owner_id": group_id,
...
def generate_agnes_image(prompt):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
        full_prompt = f"Professional illustration about {prompt}, creative, modern, high quality, no people, no nature"
        data = {
            "prompt": full_prompt,
...
def generate_pollinations_image(prompt):
    try:
        seed = random.randint(1, 1000000)
        full_prompt = f"{prompt}, creative, modern, high quality, no people, no nature"
...
def search_pexels_photo(theme):
    ...
    params = {"query": theme, "per_page": 3, "orientation": "landscape"}
    try:
...
def generate_image(group, post_id=None):
    ...
    Если передан post_id – пытается взять фото из этого поста.
    Иначе генерирует через нейросети по теме группы.
    """
    ...
    for generator in [
        ...
        img = func(theme)
        if img:
            logger.info(f"✅ Картинка сгенерирована через {name} для {group['name']}")
            return img, name
...
def monitor_music_group():
    """Проверяет новые посты в музыкальной группе и публикует анонсы"""
    ...
    while True:
        try:
            ...
            if post_info and post_info["id"] != last_id:
                # Новый пост!
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Интро-бот запущен (с мониторингом музыкальной группы)")
    
    # Запускаем планировщик для стандартных групп в отдельном потоке
    threading.Thread(target=standard_scheduler_worker, daemon=True).start()
    
    # Запускаем мониторинг музыкальной группы в отдельном потоке
    threading.Thread(target=monitor_music_group, daemon=True).start()
    
    # Обработка команд Telegram (если есть токен)
    if TELEGRAM_TOKEN:
        last_update_id = 0
        while True:
            try:
